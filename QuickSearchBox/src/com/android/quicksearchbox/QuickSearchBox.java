...
public class QuickSearchBox extends Activity
{
	...
	private static final String		TAG									= "QSB.QuickSearchBox";	//用来写出错日志的
	...
	//Runnable是个接口，定义了一个接口的引用；在Runnable()构造函数中，定义了一个函数run()，
	//该函数调用updateSuggestions(),这样做的意义是什么？
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		recordStartTime();			//记录启动时间
		if (!DBG)
			Log.d(TAG, "onCreate()");
		super.onCreate(savedInstanceState);
		setContentView(R.layout.search_activity);			//引用布局文件
		...
		//当焦点改变时，隐藏输入法
		mSuggestionsView
				.setOnFocusChangeListener(new SuggestListFocusListener());
		...
		//
		mQueryTextView
				.setOnFocusChangeListener(new QueryTextViewFocusListener());
		//
		...
		//语音搜索
		mVoiceSearchButton
				.setOnClickListener(new VoiceSearchButtonClickListener());
		//处理键盘事件
		...
		// Do this at the end, to avoid updating the list view when setSource() is called.
	...
	private void clearStartedIntoCorpusSelectionDialog()
	{
		...
		if (QuickSearchBox.INTENT_ACTION_QSB_AND_SELECT_CORPUS.equals(oldIntent
				.getAction()))
		{
	...
	public static Uri getCorpusUri(Corpus corpus)
	{
		if (corpus == null)
			return null;
		return new Uri.Builder().scheme(SCHEME_CORPUS).
											authority(corpus.getName()).build();
	}
	...
	private Corpus getCorpusFromUri(Uri uri)
	{
		if (uri == null)
			return null;
		if (!SCHEME_CORPUS.equals(uri.getScheme()))
			return null;
	...
	//返回一个QsbApplication对象
	private QsbApplication getQsbApplication()
	{
		return (QsbApplication) getApplication();		//返回应用程序
	}
	...
	//CorpusRanker：是个接口；得到一个实例化的CorpusRanker对象
	...
	//此处为调用写日志的程序，真是多次一举，直接调用就行了；此处可以被注释掉。
/*	private Logger getLogger()
	...
	protected void onStop()
	{
		...
		if (!mTookAction)
		{
			// TODO: This gets logged when starting other activities, e.g. by
			// opening he search
			// settings, or clicking a notification in the status bar.
			//getLogger().logExit(getCurrentSuggestions(), getQuery().length());
		}
		
		// Close all open suggestion cursors. The query will be redone in
		// onResume()
	...
	/**
	 * Sets the query as typed by the user. Does not update the suggestions or
	...
	{
		CharSequence q = mQueryTextView.getText();	//获得编辑框文本
		return q == null ? "" : q.toString();					//获得字符串内容
	}
	...
	{
		if (DBG)
			Log.d(TAG, "Restoring query to '" + mUserQuery + "'");
		setQuery(mUserQuery, false);
	}

	/**
	 * Sets the text in the query box. Does not update the suggestions, and does
	 * not change the saved user-entered query. {@link #restoreUserQuery()} will
	 * restore the query to the last user-entered query.
	 */
	 ...
	 * 
	 * @param selectAll
	 *            If {@code true}, selects the entire query. If {@false}
	 *            , no characters are selected, and the cursor is placed at the
	 *            end of the query.
	 */
	...
	private void updateQueryTextView(boolean queryEmpty)
	{
		if (queryEmpty)
		{
			...
			{
				mQueryTextView
						.setBackgroundResource(R.drawable.textfield_search_empty_google);
				mQueryTextView.setHint(null);
			}
			else
			{
				mQueryTextView
						.setBackgroundResource(R.drawable.textfield_search_empty);
	...
	protected void showCorpusSelectionDialog()
	{
		if (mCorpusSelectionDialog == null)
		{
			...
			mCorpusSelectionDialog
					.setOnDismissListener(new CorpusSelectorDismissListener());
			mCorpusSelectionDialog
					.setOnCorpusSelectedListener(new CorpusSelectionListener());
		}
	...
	protected void onSearchClicked(int method)
	{
		...
		// Start search；谁来实现这个createSearchIntent()方法
	...
	{
		if (DBG)
			Log.d(TAG, "Voice Search clicked");
		Corpus searchCorpus = mLauncher.getSearchCorpus(getCorpora(), mCorpus);
		if (searchCorpus == null)
			return;
		mTookAction = true;
		
		// Log voice search start，此处的代码应该被注释掉
	...
	protected boolean launchSuggestion(int position)
	{
		...
		// Launch intent
		Intent intent = mLauncher.getSuggestionIntent(suggestions, position,
				mAppSearchData);
		mLauncher.launchIntent(intent);
		return true;
	}
	...
	protected boolean onSuggestionKeyDown(int position, int keyCode,
			KeyEvent event)
	{
		...
		{
			// Moved up from the top suggestion, restore the user query and
			// focus query box
			if (DBG)
				...
				|| keyCode == KeyEvent.KEYCODE_DPAD_RIGHT)
		{
			...
			String query = getQuery();
			int cursorPos = (keyCode == KeyEvent.KEYCODE_DPAD_LEFT) ? 0 : query
					.length();
	...
	protected void hideInputMethod()
	{
		...
		if (imm != null)
		{
			imm.hideSoftInputFromWindow(mQueryTextView.getWindowToken(), 0);	//隐藏软键盘
		}
	}
	...
	private void updateSuggestions(String query)
	{
		...
		//
		Suggestions suggestions = getSuggestionsProvider().getSuggestions(query, corporaToQuery, getMaxSuggestions());
	...
	/**
	 * Handles key events on the search and voice search buttons, by refocusing
	 * to EditText.
	 */
	...
	//私有类，仅QuickSearchBox使用；关闭输入法
	private class InputMethodCloser implements
			SuggestionsView.InteractionListener
	{
	...
	private class ClickHandler implements SuggestionClickListener
	{
		...
		{
			return QuickSearchBox.this.onSuggestionLongClicked(position);
		}
	}
	
	//私有类，仅QuickSearchBox使用；
	...
	private class CorpusSelectorDismissListener implements
			DialogInterface.OnDismissListener
	{
		public void onDismiss(DialogInterface dialog)
		{
			if (DBG)
				Log.d(TAG, "Corpus selector dismissed");
			clearStartedIntoCorpusSelectionDialog();
		}
	}

	private class CorpusSelectionListener implements
			CorpusSelectionDialog.OnCorpusSelectedListener
	{
	 ...
	 */
	private class VoiceSearchButtonClickListener implements
