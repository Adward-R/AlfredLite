...
public class SuggestionsAdapter extends BaseAdapter
{
	...
	public void setSuggestions(Suggestions suggestions)
	{
		...
		// TODO: delay the change if there are no suggestions for the currently
		// visible tab.
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		SuggestionView view = mViewFactory.getSuggestionView(viewType,
				convertView, parent);
	...
	{
		if (DBG)
			Log.d(TAG, "onSuggestionsChanged(), mSuggestions=" + mSuggestions);
		SuggestionCursor cursor = getCorpusCursor(mSuggestions, mCorpus);
		changeCursor(cursor);
	}

	/**
	 * Gets the cursor containing the currently shown suggestions. The caller
	 * should not hold on to or modify the returned cursor.
	 */
	 ...
	 */
	protected SuggestionCursor getCorpusCursor(Suggestions suggestions,
			Corpus corpus)
	{
		if (suggestions == null)
			return null;
	 ...
	 * Replace the cursor.
	 * 
	 * This does not close the old cursor. Instead, all the cursors are closed
	 * in {@link #setSuggestions(Suggestions)}.
	 */
	private void changeCursor(SuggestionCursor newCursor)
	{
		if (DBG)
			Log.d(TAG, "changeCursor(" + newCursor + ")");
