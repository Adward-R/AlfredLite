...
/**
 * A {@link SuggestionsAdapter} that doesn't expose the new suggestions until
 * there are some results to show.
 */
public class DelayingSuggestionsAdapter extends SuggestionsAdapter
{
	...
	public void setSuggestions(Suggestions suggestions)
	{
		...
		if (shouldPublish(suggestions))
		{
			if (DBG)
				Log
						.d(TAG, "Publishing suggestions immediately: "
								+ suggestions);
		...
		{
			if (DBG)
				Log.d(TAG, "Delaying suggestions publishing: " + suggestions);
	...
	private void setPendingSuggestions(Suggestions suggestions)
	{
		...
		{
			mPendingSuggestions
					.unregisterDataSetObserver(mPendingDataSetObserver);
	...
	protected void onPendingSuggestionsChanged()
	{
		...
		{
			if (DBG)
				Log.d(TAG, "Suggestions now available, publishing: "
						+ mPendingSuggestions);
