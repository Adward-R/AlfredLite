...
/**
 * A SuggestionCursor that allows shortcuts to be updated by overlaying with
...
class ShortcutCursor extends ListSuggestionCursor
{
	...
	public ShortcutCursor(int maxShortcuts,
			CursorBackedSuggestionCursor shortcuts)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				if (DBG)
					Log.d(TAG, "Skipping shortcut " + i);
			}
		}
	}

	/**
	 * Updates this SuggestionCursor with a refreshed result from another. Since
	 * this modifies the cursor, it should be called on the UI thread. This
	 * class assumes responsibility for closing refreshed.
	 */
	public void refresh(Source source, String shortcutId,
			SuggestionCursor refreshed)
	{
		if (DBG)
			Log.d(TAG, "refresh " + shortcutId);
		if (mClosed)
		{
			if (refreshed != null)
			{
		...
		{
			moveTo(i);
			if (shortcutId.equals(getShortcutId())
