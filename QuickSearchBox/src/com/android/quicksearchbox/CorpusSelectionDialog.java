...
public class CorpusSelectionDialog extends Dialog
{
	...
	{
		super.onStart();
		CorporaAdapter adapter = CorporaAdapter.createGridAdapter(
				getViewFactory(), getCorpusRanker());
		setAdapter(adapter);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)
	{
		...
		{
			int selectedRow = mCorpusGrid.getSelectedItemPosition()
					/ NUM_COLUMNS;
			if (selectedRow <= 0)
			{
				cancel();
	...
	{
		QuickSearchBox searchActivity = getSearchActivity();
	...
	//
	private QuickSearchBox getSearchActivity()
	{
		return (QuickSearchBox) getOwnerActivity();
	}
	...
	//结果单击响应
	private class CorpusClickListener implements
