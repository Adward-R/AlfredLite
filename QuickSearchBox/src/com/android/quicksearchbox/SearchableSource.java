...
public class SearchableSource implements Source
{
	...
	// TODO: This should be exposed or moved to android-common, see
	// http://b/issue?id=2440614
	// The extra key used in an intent to the speech recognizer for in-app voice
	// search.
			...
			throws NameNotFoundException
	{
		...
		PackageInfo pkgInfo = pm.getPackageInfo(componentName.getPackageName(),
				0);
	...
	/**
	 * Checks if the current process can read the suggestion provider in this
	 * source.
	 */
	public boolean canRead()
	{
		...
		// if content path provided, insert it now,获得搜索结果的路径
	 ...
	 * 检查当前进程能否读给定内容的URI
	 * TODO: Shouldn't this be a PackageManager / Context / ContentResolver
	 * method?
	 */
	private boolean canRead(Uri uri)
	{
		...
		if (provider == null)
		{
			Log.w(TAG, getName() + " has bad suggestion authority "
					+ uri.getAuthority());
			return false;
		}
		...
		if (mContext.checkPermission(readPermission, pid, uid) == PackageManager.PERMISSION_GRANTED)
		{
		...
		{
			// We don't have the readPermission, and there are no
			// pathPermissions
			if (DBG)		//
		...
		for (PathPermission perm : pathPermissions)
		{
			...
			if (pathReadPermission != null
					&& perm.match(path)
					&& mContext.checkPermission(pathReadPermission, pid, uid) == PackageManager.PERMISSION_GRANTED)
			{
		...
		if (DBG)
			Log.d(TAG, "Missing " + readPermission
					+ " and no path permission applies");
		return false;
	}
	...
	private IconLoader createIconLoader(Context context, String providerPackage)
	{
		if (providerPackage == null)
			return null;
		return new CachingIconLoader(new PackageIconLoader(context,
				providerPackage));
	}
	...
	public Uri getSourceIconUri()
	{
		...
		return Util.getResourceUri(getContext(), mActivityInfo.applicationInfo,
				resourceId);
	}
	...
	public static Intent createSourceSearchIntent(ComponentName activity,
			String query, Bundle appData)
	{
		if (activity == null)
		{
	...
	public SourceResult getSuggestions(String query, int queryLimit)
	{
		try
		{
			Cursor cursor = getSuggestions(mContext, mSearchable, query,
					queryLimit);
			if (DBG)
				Log.d(TAG, toString() + "[" + query + "] returned.");
	...
	public SuggestionCursor refreshShortcut(String shortcutId, String extraData)
	{
		...
		try
		{
			cursor = getValidationCursor(mContext, mSearchable, shortcutId,
					extraData);
			if (DBG)
				Log.d(TAG, toString() + "[" + shortcutId + "] returned.");
		...
		catch (RuntimeException ex)
		{
			...
			// TODO: Should we delete the shortcut even if the failure is
			// temporary?
			return null;
		}
	}
	...
	/**
	 * This is a copy of
	 * {@link SearchManager#getSuggestions(SearchableInfo, String)}.
	 */
	private static Cursor getSuggestions(Context context,
			SearchableInfo searchable, String query, int queryLimit)
	{
		if (searchable == null)
		{
			return null;
		...
		return context.getContentResolver().query(uri, null, selection,
				selArgs, null);
	}

	private static Cursor getValidationCursor(Context context,
			SearchableInfo searchable, String shortcutId, String extraData)
	{
		String authority = searchable.getSuggestAuthority();
		if (authority == null)
		{
		...
		Uri uri = uriBuilder.appendQueryParameter(
				SearchManager.SUGGEST_COLUMN_INTENT_EXTRA_DATA, extraData)
				.build();
		if (DBG)
			Log.d(TAG, "Requesting refresh " + uri);
		// finally, make the query
	...
	private CharSequence getText(int id)
	{
		if (id == 0)
			return null;
		return mContext.getPackageManager().getText(mActivityInfo.packageName,
				id, mActivityInfo.applicationInfo);
	}
