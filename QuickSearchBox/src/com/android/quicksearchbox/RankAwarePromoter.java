...
public class RankAwarePromoter implements Promoter
{
			...
			ListSuggestionCursor promoted)
	{
		...
		// Split non-empty results into default sources and other, positioned at
		// first suggestion
		...
		if (maxPromoted > 0 && !defaultResults.isEmpty())
		{
			int slotsToFill = Math.min(getSlotsAboveKeyboard()
					- promoted.getCount(), maxPromoted);
			if (slotsToFill > 0)
			{
				int stripeSize = Math.max(1, slotsToFill
						/ defaultResults.size());
				maxPromoted -= roundRobin(defaultResults, slotsToFill,
						stripeSize, promoted);
			}
		}
		...
		if (DBG)
			Log.d(TAG, "Returning " + promoted.toString());
	}
	 ...
	 * 
	 * @param results
	 *            the list of CorpusResults from which to promote. Exhausted
	 *            CorpusResults are removed from the list.
	 * @param maxPromoted
	 *            maximum number of suggestions to promote.
	 * @param stripeSize
	 *            number of suggestions to take from each corpus.
	 * @param promoted
	 *            the list to which promoted suggestions are added.
	 * @return the number of suggestions actually promoted.
	 */
	private int roundRobin(LinkedList<CorpusResult> results, int maxPromoted,
			int stripeSize, ListSuggestionCursor promoted)
	{
		int count = 0;
		if (maxPromoted > 0 && !results.isEmpty())
		{
			for (Iterator<CorpusResult> iter = results.iterator(); count < maxPromoted
					&& iter.hasNext();)
			{
	...
	/**
	 * Copies suggestions from a SuggestionCursor to the list of promoted
	 * suggestions.
	 * 
	 * @param cursor
	 *            from which to copy the suggestions
	 * @param count
	 *            maximum number of suggestions to copy
	 * @param promoted
	 *            the list to which to add the suggestions
	 * @return the number of suggestions actually copied.
	 */
	private int promote(SuggestionCursor cursor, int count,
			ListSuggestionCursor promoted)
	{
