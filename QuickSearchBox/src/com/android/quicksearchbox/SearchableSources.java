...
public class SearchableSources implements Sources
{
	 ...
	 * 
	 * @param context
	 *            Used for looking up source information etc.
	 */
	...
	public void load()
	{
		...
		intentFilter
				.addAction(SearchManager.INTENT_ACTION_SEARCH_SETTINGS_CHANGED);
	...
	private void addSearchableSources()
	{
		...
		for (SearchableInfo searchable : searchables)
		{
			...
			{
				if (DBG)
					Log.d(TAG, "Created source " + source);
				addSource(source);
			}
		}
	}
	...
	{
		// Looks for an activity in the current package that handles
		// ACTION_WEB_SEARCH.
