...
public class QueryTask<C extends SuggestionCursor> implements NamedTask
{
	 ...
	 *            Query to run.
	 * @param queryLimit
	 *            The number of suggestions to ask each provider for.
	 * @param provider
	 *            The provider to ask for suggestions.
	 * @param handler
	 *            Handler that {@link Consumer#consume} will get called on. If
	 *            null, the method is called on the query thread.
	 * @param consumer
	 *            Consumer to notify when the suggestions have been returned.
	 */
	public QueryTask(String query, int queryLimit,
			SuggestionCursorProvider<C> provider, Handler handler,
			Consumer<C> consumer)
	{
		mQuery = query;
