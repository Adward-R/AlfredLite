...
/**
 * Holds data for each suggest item including the display data and how to launch
 * the result. Used for passing from the provider to the suggest cursor.
 * 拥有每个搜索项的数据，包括数据显示和运行搜索结果。
 */
public class SuggestionData
{
	private final Source	mSource;		//Source是个interface
	...
	@VisibleForTesting
	public SuggestionData setSpinnerWhileRefreshing(
	...
	public String getSuggestionKey()
	{
		...
		// calculating accurate size of string builder avoids an allocation vs
		// starting with the default size and having to expand.
		int size = action.length() + 2 + data.length() + query.length();
		
		return new StringBuilder(size).append(action).append('#').append(data)
	...
	public int hashCode()
	{
		...
		result = prime * result
				+ ((mIntentAction == null) ? 0 : mIntentAction.hashCode());
		result = prime * result
				+ ((mIntentData == null) ? 0 : mIntentData.hashCode());
		result = prime * result
				+ ((mIntentExtraData == null) ? 0 : mIntentExtraData.hashCode());
		result = prime * result
				+ ((mLogType == null) ? 0 : mLogType.hashCode());
		result = prime * result
				+ ((mShortcutId == null) ? 0 : mShortcutId.hashCode());
		result = prime * result + ((mSource == null) ? 0 : mSource.hashCode());
		result = prime * result + (mSpinnerWhileRefreshing ? 1231 : 1237);
		result = prime * result
				+ ((mSuggestionQuery == null) ? 0 : mSuggestionQuery.hashCode());
