...
public class DefaultCorpusRanker extends AbstractCorpusRanker
{
	...
	private final ShortcutRepository	mShortcuts;	//又是个接口

	public DefaultCorpusRanker(Corpora corpora, ShortcutRepository shortcuts)
	{
		super(corpora);			//调用AbstractCorpusRanker的构造函数
	...
	private static class CorpusComparator implements Comparator<Corpus>
	{
		...
		public CorpusComparator(Corpora corpora,
	...
	public List<Corpus> rankCorpora(Corpora corpora)
	{
		...
		if (DBG)
			Log.d(TAG, "Ranking: " + enabledCorpora);
		...
		if (DBG)
			Log.d(TAG, "Click scores: " + clickScores);
		if (DBG)
			Log.d(TAG, "Ordered: " + ordered);
		return ordered;
	}
}
