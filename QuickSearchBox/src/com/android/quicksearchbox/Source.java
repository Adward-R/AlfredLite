...
{
	/**
	 * Gets the name of the activity that this source is for. When a suggestion
	 * is clicked, the resulting intent will be sent to this activity.
	 * 获得一个搜索结果被单击后产生的activity名称
	 */
	ComponentName getComponentName();

	/**
	 * Gets the version code of the source. This is expected to change when the
	 * app that this source is for is upgraded.
	 */
	int getVersionCode();
	...
	/**
	 * 
	 * Note: this does not guarantee that this source will be queried for
	 * queries of this length or longer, only that it will not be queried for
	 * anything shorter.
	 * 
	...
	/**
	 * Indicates whether a source should be invoked for supersets of queries it
	 * has returned zero results for in the past. For example, if a source
	 * returned zero results for "bo", it would be ignored for "bob".
	 * 
	 * If set to <code>false</code>, this source will only be ignored for a
	 * single session; the next time the search dialog is brought up, all
	 * sources will be queried.
	 * 
	 * @return <code>true</code> if this source should be queried after
	 ...
	 *            The user query.
	 * @param queryLimit
	 *            An advisory maximum number of results that the source should
	 *            return.
	 ...
	 *         cursor is empty or <code>null</code>, the shortcut will be
	 *         removed.
	 */
	...
	/**
	 * Checks whether the text in the query field should come from the
	 * suggestion intent data.
	 */
