...
public class Suggestions
{
	...
	/**
	 * The observers that want notifications of changes to the published
	 * suggestions. This object may be accessed on any thread.
	 */
	private final DataSetObservable			mDataSetObservable	= new DataSetObservable();
	/**
	 * All {@link SuggestionCursor} objects that have been published so far, in
	 * the order that they were published. This object may only be accessed on
	 * the UI thread.
	 * 用来存储查找结果？以CorpusResult为元素的链表
	 * */
	 ...
	 * 
	 * @param expectedCorpusCount
	 *            The number of sources that are expected to report.
	 */
	public Suggestions(Promoter promoter, int maxPromoted, String query,
			int expectedCorpusCount)
	{
		mPromoter = promoter;
	...
	/**
	 * Gets the set of corpora that have reported results to this suggestions
	 * set.
	 * 
	...
	public Set<Corpus> getIncludedCorpora()
	{
		...
		//ArrayList<CorpusResult>	mCorpusResults;
		for (CorpusResult result : mCorpusResults)
		{
			corpora.add(result.getCorpus());
		}
		return corpora;		//HashSet<Corpus> 等价于 Set<Corpus>
	}
	...
	protected void finalize()
	{
		if (!mClosed)
		{
			Log.e(TAG, "LEAK! Finalized without being closed: Suggestions["
					+ mQuery + "]");
		}
	}

	/**
	 * Checks whether all sources have reported. Must be called on the UI
	 * thread, or before this object is seen by the UI thread.
	 */
	public boolean isDone()
	{
		// TODO: Handle early completion because we have all the results we
		// want.
		return mCorpusResults.size() >= mExpectedCorpusCount;
	}

	/**
	 * Sets the shortcut suggestions. Must be called on the UI thread, or before
	 * this object is seen by the UI thread.
	 * 
	 * @param shortcuts
	 *            The shortcuts.
	 */
	...
	{
		if (DBG)
			Log.d(TAG, "setShortcuts(" + shortcuts + ")");
	...
	/**
	 * Gets the number of source results. Must be called on the UI thread, or
	 * before this object is seen by the UI thread.
	 */
	public int getSourceCount()
	{
		if (mClosed)
		{
			throw new IllegalStateException(
					"Called getSourceCount() when closed.");
		}
