...
public class GoogleSearch extends Activity
{
	...
	// "source" parameter for Google search requests from unknown sources (e.g.
	// apps). This will get
	 ...
	 * com.google.android.providers.enhancedgooglesearch.Launcher. If you are
	 * changing this make sure you change both.
	 */
	private void handleWebSearchIntent(Intent intent)
	{
		...
		// If the caller specified a 'source' url parameter, use that and if not
		// use default.
		...
		// The browser can pass along an application id which it uses to figure
		// out which
		// window to place a new search into. So if this exists, we'll pass it
		// back to
		// the browser. Otherwise, add our own package name as the application
		// id, so that
		// the browser can organize all searches launched from this provider
		// together.
		String applicationId = intent
		...
		try
		{
			String searchUri = googleSearchUrlBase + "&source=android-"
					+ source + "&q=" + URLEncoder.encode(query, "UTF-8");
			Intent launchUriIntent = new Intent(Intent.ACTION_VIEW, Uri
					.parse(searchUri));
			launchUriIntent.putExtra(Browser.EXTRA_APPLICATION_ID,
					applicationId);
